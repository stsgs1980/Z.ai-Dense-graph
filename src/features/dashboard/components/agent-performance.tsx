...
import { ROLE_GROUPS, TOP_PERFORMERS, STATUS_DISTRIBUTION, SPARKLINE_DATA, PERFORMANCE_METRICS } from '@/shared/config/dashboard-constants'
...
function TopPerformersSection({ topPerformers, barWidths }: { topPerformers: typeof TOP_PERFORMERS; barWidths: number[] }) {
  return (
...
function StatusDonut({ statusDistribution }: { statusDistribution: typeof STATUS_DISTRIBUTION }) {
  const donutRadius = 50
  const donutStroke = 10
  const donutCircumference = 2 * Math.PI * donutRadius
  const totalAgents = statusDistribution.reduce((sum, s) => sum + s.count, 0)
  const donutSegments = statusDistribution.filter(s => s.count > 0).reduce<Array<{ label: string; count: number; color: string; segmentLength: number; offset: number }>>((acc, status) => {
  ...
  return (
    <div className="rounded-xl p-4 sm:p-6" style={{ background: 'rgba(45, 45, 45, 0.3)', border: '1px solid rgba(51, 51, 51, 0.5)' }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <TopPerformersSection topPerformers={topPerformers} barWidths={barWidths} />
        <StatusDonut statusDistribution={statusDistribution} />
      </div>
      <MetricsSection />
    </div>
  )
