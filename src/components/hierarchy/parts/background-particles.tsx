...
function drawFrame(ctx: CanvasRenderingContext2D, contours: Contour[], markers: Array<{ x: number; y: number }>, canvasW: number, canvasH: number) {
  ...
  for (const m of markers) {
    ctx.strokeStyle = 'rgba(51, 51, 51, 0.3)'; ctx.lineWidth = 0.8; ctx.beginPath()
    ctx.moveTo(m.x - 3, m.y); ctx.lineTo(m.x + 3, m.y); ctx.moveTo(m.x, m.y - 3); ctx.lineTo(m.x, m.y + 3)
    ctx.stroke()
  }
  for (const c of contours) {
    ...
    for (let ri = 0; ri < c.rings.length; ri++) {
      const baseR = c.rings[ri]; ctx.beginPath(); const segments = 48
      for (let s = 0; s <= segments; s++) {
        ...
        const r = baseR + irregularity; const px = c.cx + Math.cos(angle) * r; const py = c.cy + Math.sin(angle) * r * 0.85
        if (s === 0) ctx.moveTo(px, py); else ctx.lineTo(px, py)
      }
      ctx.closePath(); ctx.strokeStyle = `rgba(6, 182, 212, ${currentOpacity * (1 - ri * 0.12)})`; ctx.lineWidth = 0.6; ctx.stroke()
    }
  }
}
...
export function BackgroundParticles() {
  ...
  useEffect(() => {
    ...
    if (contoursRef.current.length === 0) {
      ...
      contoursRef.current = contours; markersRef.current = markers
    }
    const animate = () => { drawFrame(ctx, contoursRef.current, markersRef.current, canvas.width, canvas.height); animationRef.current = requestAnimationFrame(animate) }
    animate()
